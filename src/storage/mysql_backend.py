...
class MySQLBackend(StorageBackend):
    ...
    async def get_user_by_name(self, username: str) -> User | None:
        row = await self._one("SELECT * FROM users WHERE username=%s", (username,))
    ...
    async def update_user(self, user: User) -> None:
        ...
        await self._execute(
            "UPDATE users SET username=%s,default_model=%s,default_preset_id=%s,updated_at=%s WHERE id=%s",
            (user.username, user.default_model, user.default_preset_id, user.updated_at, user.id),
        )
    ...
    async def create_session(self, session: Session) -> Session:
        session.id = await self._execute(
            "INSERT INTO sessions(user_id,title,model_name,preset_id,total_prompt_tokens,"
            "total_completion_tokens,created_at,updated_at) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)",
            (
                session.user_id,
                session.title,
                session.model_name,
    ...
    async def get_session(self, session_id: int) -> Session | None:
        row = await self._one("SELECT * FROM sessions WHERE id=%s", (session_id,))
    ...
    async def update_session(self, session: Session) -> None:
        ...
        await self._execute(
            "UPDATE sessions SET title=%s,model_name=%s,preset_id=%s,total_prompt_tokens=%s,total_completion_tokens=%s,updated_at=%s WHERE id=%s",
            (
                session.title,
                session.model_name,
                session.preset_id,
    ...
    async def add_message(self, message: Message) -> Message:
        message.id = await self._execute(
            "INSERT INTO messages(session_id,role,content,prompt_tokens,completion_tokens,created_at) VALUES(%s,%s,%s,%s,%s,%s)",
            (
                message.session_id,
                message.role,
                message.content,
    ...
    async def search_messages(self, user_id: int, keyword: str) -> list[Message]:
        return [
            Message(**row)
            for row in await self._all(
                "SELECT m.* FROM messages m JOIN sessions s ON s.id=m.session_id WHERE s.user_id=%s AND m.content LIKE %s ORDER BY m.id",
                (user_id, f"%{keyword}%"),
            )
        ]

    async def get_preset_by_id(self, preset_id: int) -> Preset | None:
        row = await self._one("SELECT * FROM presets WHERE id=%s", (preset_id,))
        return Preset(**row) if row else None

    async def save_preset(self, preset: Preset) -> Preset:
        preset.updated_at = datetime.now(timezone.utc)
        if not preset.id:
            preset.id = await self._execute(
                "INSERT INTO presets(user_id,name,description,system_prompt,is_builtin,created_at,updated_at) VALUES(%s,%s,%s,%s,%s,%s,%s)",
                (
                    preset.user_id,
                    preset.name,
                    preset.description,
            ...
            await self._execute(
                "UPDATE presets SET name=%s,description=%s,system_prompt=%s,is_builtin=%s,updated_at=%s WHERE id=%s",
                (
                    preset.name,
                    preset.description,
                    preset.system_prompt,
    ...
    async def list_presets(self, user_id: int) -> list[Preset]:
        return [
            Preset(**row)
            for row in await self._all(
                "SELECT * FROM presets WHERE user_id IS NULL OR user_id=%s ORDER BY is_builtin DESC,id",
                (user_id,),
            )
        ]
    ...
    async def set_user_config(self, config: UserConfig) -> None:
        await self._execute(
            "INSERT INTO user_configs(user_id,`key`,value,updated_at) VALUES(%s,%s,%s,%s) "
            "ON DUPLICATE KEY UPDATE value=VALUES(value),updated_at=VALUES(updated_at)",
            (config.user_id, config.key, config.value, config.updated_at),
        )
