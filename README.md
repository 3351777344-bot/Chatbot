...
Streamlit 聊天界面、多轮会话与 SQLite 持久化功能；本次只完成 Step 1 的项目初始化和
工程化配置，没有删除、重写或提前扩展现有业务代码。
...
入口验证成功时会显示项目名 `langchain-chat`、版本 `0.1.0`、Python 版本、当前 Step 1
进度和启动成功提示。
...
3. 执行 `uv run python src/main.py` 验证 Step 1 工程化入口。
